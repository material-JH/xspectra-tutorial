...
- Follow-up screen cleanup hides the right-side header context label and the footer title in browser presentation mode. The footer now keeps the centered progress bar and restores the slide number at the right edge.
- Header typography now matches `bash_tutorial/docs/linux_command_tutorial_presentation.html`: 28px uppercase label, 320px underline, and larger two-dot section marker.
...
1. Footer alignment is patched for screen presentation.
   ...
   - After the follow-up cleanup, the footer title is hidden in screen mode, the slide number is visible at the right edge, and the footer is still hidden in print/PDF, matching the previous print behavior.

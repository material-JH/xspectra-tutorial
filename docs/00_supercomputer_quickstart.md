...
주의: 어떤 클러스터는 `pw.x`, `xspectra.x` 실행 파일만 설치하고 `XSpectra/tools/upf2plotcore.sh`는 설치하지 않습니다. Diamond와 SrTiO3 유사퍼텐셜은 이 저장소에 포함되어 있으므로 보통 따로 다운로드하지 않아도 됩니다. `upf2plotcore.sh`가 없을 때는 [QE source tree와 유사퍼텐셜 준비](00_qe_source_and_pseudopotentials.md)를 참고해서 QE source tree를 내려받고 `TOOLS_DIR`를 설정하세요.

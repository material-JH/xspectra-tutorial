...
PBS/Slurm 작업으로 계산했다면 작업이 끝난 뒤 로그인 노드에서 그림을 만듭니다.
...
`plot_spectra.py`는 `diamond/diamond.xspectra.dat`, `diamond/diamond.xspectra_replot.dat`, `diamond/diamondh.xspectra.dat`가 있으면 방금 계산한 결과를 그립니다. 아직 계산 결과가 없으면 `reference_output/diamond/`의 기준 파일을 그립니다.
